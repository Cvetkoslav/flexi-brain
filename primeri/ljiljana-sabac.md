...
# Primer konverzacije — Ljiljana Lekić (Šabac, 73 god)

**Povezano:** [[INDEX]] · [[states/UBJEDJIVANJE]] · [[objekcije/popust]] · [[objekcije/nema-para]] · [[zatvaranje/odgada]]

## Profil klijenta

- **Ime:** Ljiljana Lekić
- **Lokacija:** Šabac (15000), Ul. Leonarda da Vinčija 43/7
- **Godine:** 73
- **Penzija:** 31 000 RSD (10.04.)
- **Lekovi:** Concor cor 2,5mg, Triplixan 10/5mg i 2,5mg (srce + pritisak)
- **Simptom:** bol u kolenu, Bekerova cista, **5 godina**
- **Već probala:** Aktiv+
- **Telefon:** 066/341 471

## Pun transkript (doslovno iz 4 screenshot-a)

### Screenshot 1 — POCETAK + PRIKUPLJANJE + PREPORUKA
...
Cena tog kompleta je 2800 dinara i namenjen je da traji 30 dana.
...
| Fragment konverzacije | KB fajl koji pokriva |
|---|---|
| "Pre nego da Vam damo sav info, par pitanja..." | `pravila/prva-poruka` + `states/POCETAK` |
| "Gde Vam se javljaju bolovi, koliko traju?" (pitanje #1) | `states/PRIKUPLJANJE` |
| "Koliko godina, lekovi?" (pitanje #2+3) | `states/PRIKUPLJANJE` + `stil-govora/max-3-pitanja` |
| Preporuka dva-dela (kapsule iznutra + balzam spolja) | `proizvodi/balzam-kapsule` + `states/PREPORUKA` |
| "5-6 dana ne osećaju bol, 8/10 prezadovoljni" | `proizvodi/trajanje-rezultata` |
| "Da li Vam je prihvatljiva naša ponuda?" | `states/PREPORUKA` (close posle cene) |
| "Prirodni, Rusija, ministarstvo zdravlja" | `proizvodi/certifikati-sigurnost` + UBJEDJIVANJE A1 |
| "Fiskalni račun u paketu" | `struktura/dostava` |
| Odbijanje popusta + "ja bih i duplo koristila" | `objekcije/popust` (drugi pokušaj) + UBJEDJIVANJE A2 |
| "Ugovor sa poštom, konkurencija 15-20 KM" | `struktura/dostava` + UBJEDJIVANJE A5 |
| "Nigde loših komentara" | UBJEDJIVANJE A3 + A4 |
| "1,6 KM dnevno" (90 din RSD) | `pravila/sidrenje-vrijednosti` + UBJEDJIVANJE A8 |
| Dijagnoza prigovora ("muči cena ili delotvornost?") | `states/UBJEDJIVANJE` Faza 2 + `zatvaranje/razmislicu` |
| "Slažem se ali ne mogu sad" → prelaz na rezervaciju | `objekcije/nema-para` + UBJEDJIVANJE Faza 3 |
| "Okvirno kada bi to bilo, bez obaveze" | `zatvaranje/odgada` + UBJEDJIVANJE Faza 3 |
| Rezervacija close + traženje podataka | `zatvaranje/klijent-kaze-da` + `states/ZATVARANJE` |

## Ključne lekcije

1. **Kvalifikacija ide SAMO u POCETAK/PRIKUPLJANJE** — 3 pitanja (lokacija+trajanje, godine, lekovi). Posle cene — **nikad ne vraćaj** na kvalifikaciju.
2. **Posle cene ide UBJEDJIVANJE** — argumenti + close, ne ponovna pitanja.
3. **Close pitanje u svakoj poruci** — "Da li Vam je prihvatljivo?", "Da li se slažete?".
4. **Ivana je pitala close 3 puta zaredom** (screenshot 4) — bot treba da persistira, ne odustaje na prvi oklevaj.
5. **Lični primer je moćniji od teorije** — "ja bih koristila i duplo da kosta, nakon 6 povreda ostala pokretna".
6. **Odbijanje popusta KATEGORIČKI** — "ne određujem cenu" + lični primer, nikad popuštanje.
7. **"Nema para" nije "skupo"** — ne sidri 1,6 KM dnevno. Idi direktno na rezervaciju.
8. **Rezervacija = zadnji close** — kad klijent da datum → odmah adresa + telefon. Kao da je pristao.
